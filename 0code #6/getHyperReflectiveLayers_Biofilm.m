...
function paths = getHyperReflectiveLayers_Biofilm(inputImg, constants, membraneIntensity)
...
% roiImgmW(gy >= 0.1 | abs(gx)>=0.1) = 1 ;
roiImgmW(gy >= 0.1) = 1 ;
roiImgW = zeros(szImgNew);
% roiImgW(gyMinus >= 0.1 | abs(gx)>=0.1) = 1 ;
roiImgW(gyMinus >= 0.1 ) = 1 ;

h = fspecial('average',[15,15]);
roiImgmW = imfilter(roiImgmW, h, 'same','symmetric');
roiImgW = imfilter(roiImgW, h, 'same','symmetric');
roiImgmW(roiImgmW>1e-3) = 1;
roiImgW(roiImgW>1e-3) = 1;
