function [maskBiofilm, params] = getMask_biofilm(img,params)
%%
%
...
layerSegmentationOrder = {'roughILMandISOS' 'ilm'};
...
for layerInd = 1:numel(layerSegmentationOrder)        
    [layers, ~] = getBiofilmCore(layerSegmentationOrder{layerInd},imgNew,params,layers);
end
...
maskBiofilm = img_full;

Layerx = paths(1).pathX;
Layery = paths(1).pathY;
Layerx(Layery<2) = []; Layerx = Layerx(1:size(img_full,2));
Layery(Layery<2) = []; Layery = Layery(1:size(img_full,2));
for j = 1:size(img_full,2)
    maskBiofilm(Layerx(j)+1:end,j) = 1;
end