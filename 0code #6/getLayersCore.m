function [rPaths, img] = getLayersCore(layerName,img,params,rPaths)
...
switch layerName
    ...
    case {'roughILMandISOS'}
        ...
        imgOld = img(:,2:end-1);
        pathsTemp = getHyperReflectiveLayers(imgOld,params.roughILMandISOS,params.membraneIntensity);                 
...
%             endInd0 = rPaths(strcmp('isos',{rPaths.name})).pathX(indPathX(1));
        ...
        case {'ilm'} % biomass compact upper boundary
            ...
            indPathX_membrane = find(rPaths(strcmp('isos',{rPaths.name})).pathY==k);
            membrane_PathX = rPaths(strcmp('isos',{rPaths.name})).pathX(indPathX_membrane(1));
            
            endInd = min(rPaths(strcmp('ilm',{rPaths.name})).pathX(indPathX(1)) + params.ilm_1, membrane_PathX-3);             
...
if isSmooth
    switch layerName
        case {'rpe' 'isos'}       
            ...
            lambda = 1E-7; %small means really smooth
            pathXpoly = pathX;
            pathYpoly = pathY;
