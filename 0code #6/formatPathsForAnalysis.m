...
for i = 1:numel(imageLayer);
    ...
    for j = 1:numel(imageLayer(i).layers),

       
        %make sure subscript-Y is inbound image, and left shift subscript-y
        indValidPath = find(imageLayer(i).layers(j).pathY ~=1 & ...
                       imageLayer(i).layers(j).pathY ~= szImg(2)+2);
        
        pathX = imageLayer(i).layers(j).pathX(indValidPath);
        pathY = imageLayer(i).layers(j).pathY(indValidPath)-1; 
        ...
        if params.isResize(1)
                ...
                imageLayer(i).layers(j).pathYAnalysis = 1:szImg(2)/params.isResize(2);        
                imageLayer(i).layers(j).pathXAnalysis = nan([1 szImg(2)/params.isResize(2)]);
                ...
                imageLayer(i).layers(j).pathXAnalysis(:) = round(interp1(pathYNew,pathXNew,1:szImg(2)/params.isResize(2),'linear','extrap'));
